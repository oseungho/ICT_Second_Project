...
<body>
	...
	<div class="main-form" style="margin-top: 50px">
		<div class="main-area">
			...
			<form method="post" action="<c:url value="/edit/complete.do?no=${no}"/>">
				<div class="int-area">
					<label for="title" class="chk-la">Title</label> 
					<input type="text" class="titletext" placeholder="input title" name="title" value="${title }" required>
				</div>
				<div class="int-area second">
					<label class="chk-la" for="bbsname">Post Page</label>
					<select class="chk-select" name="bbsname" required>
						<c:choose>
						<c:when test="${bbsname=='PYTHON'}">
							<option value="PYTHON" selected>PYTHON</option>
						</c:when>
						<c:when test="${bbsname=='HTML'}">
							<option value="HTML">HTML</option>
						</c:when>
						<c:when test="${bbsname=='CSS'}">
				...
				<div class="int-area">
					...
					<textarea class="text text-area" rows="10" name="content">${content }</textarea>
				</div>
